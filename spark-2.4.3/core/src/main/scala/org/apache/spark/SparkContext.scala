 ...
 *
 * 参考：https://www.cnblogs.com/xia520pi/p/8609602.html#_label2_0
 *  SparkContext的初始化步骤：
 * 1、创建Spark的执行环境SparkEnv
 * 2、创建并且初始化Spark UI
 * 3、hadoop相关配置以及Executor环境变量的设置
 * 4、创建任务调度TaskScheduler
 * 5、创建和启动DAGScheduler
 * 6、TaskScheduler的启动
 * 7、初始化管理器BlockManager(BlockManager 是存储体系的主要组件之一)
...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    /**
     * 
     * SparkConf 在初始化了之后要设置 master 和app.name
     */
    ...
    /**
     * 
     * 第七步： taskScheduler的启动，主要任务：taskSchedulerImp backend.start()
     */
