...
private[spark] class DAGScheduler(
   ...
   *
   * helm
   * 采用的是深度优先遍历找到 Action 算子的父依赖中的宽依赖
  ...
  /**
   * helm
   *    RDD DAG划分Stages：Stage的划分是从最后一个Stage开始逆推的，每遇到一个宽依赖处，就分裂成另外一个Stage
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
       ...
       *          ResultStage 执行完了之后，就按照程序的要求，把数据持久化，或打印输出
       *   4、Task
       */
    } catch {
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
   ...
   *   这些事件到底是什么，转到 DAGSchedulerEvent 来看详细注释
   */
