...
/**
 * 注释 
 * Spark 的StandAlone 集群的主节点实现类
 *
 * 由于Master 是RpcEndpoint 的子类：
 * 所以必然有对应的声明周期方法：
 * 1、onStart
 * 2、receive
 * 3、onStop
...
private[deploy] class Master(
  ...
  /**
   *  workers节点信息
   * 1、WorkInfo 一个 worker 节点的各种信息被封装成这样的一个对象
   * 2、workers 用来存放 上线的 worker 节点
   */
  ...
  override def onStart(): Unit = {
     ...
     * 最终会调用 timeOutDeadWorker()，用于检测超时的Worker，并移除
     * 默认60s
     */
    ...
    val (persistenceEngine_, leaderElectionAgent_) = RECOVERY_MODE match {
      ...
      /**
       * 
       * FILESYSTEM 的方式在Master 出现故障后需要手动启动机器，
       * 机器启动后会立即成为Active 级别的 Master 来对外提供服务
       * 接收应用程序提交的请求，接收新的 Job 运行的请求
       */
      case "FILESYSTEM" =>
        val fsFactory =
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case RegisterWorker(
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        if (registerWorker(worker)) {
          ...
          //  注册成功，将 worker信息注册到 Master 的 workers set中
          persistenceEngine.addWorker(worker)

          /**
           *  返回注册成功消息
           * 1、workerRef 是Worker 的 endpoint de 代理
           * 2、给 worker 返回一个RegisteredWorker 消息
           */
    ...
    case RegisterApplication(description, driver) =>
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        /**
         *  给Driver 端返回一个注册成功的消息：RegisteredApplication
         */
     ...
     * 
     * 心跳
     * master启动后，会等待worker启动好了后，过来汇报
     * worker注册成功后，会发送过来心跳消息
     */
    case Heartbeat(workerId, worker) =>
      idToWorker.get(workerId) match {
        //  每一个worker 发送心跳来了之后，master都会更新最后一次心跳的时间
  ...
  /**
   * 
   * 在master 中，每一个worker上线后，向master注册， 注册成功，执行schedule()调度方法，启动driver 或者 executor
   * 1、有可能：launchDriver(worker,driver)
   * 2、有可能：startExecutorsOnWorkers
   */
  ...
  private def registerApplication(app: ApplicationInfo): Unit = {
    ...
    // 添加 application 到 waitingApps 队列中
    waitingApps += app
  }
...
private[deploy] object Master extends Logging {
  ...
  /**
   * 注释 
   * spark-class 传入参数：--port 7077 --webui-port 8080
   */
  def main(argStrings: Array[String]) {
    ...
    // 注释 
    // 创建conf对象，读取Spark的默认配置参数
    val conf = new SparkConf

    /**
     * 注释 
     * 解析参数
     * 类似于构造函数（属性赋值，代码块）的代码都会执行
     * 不能执行的：内部类，方法
     */
  ...
  def startRpcEnvAndEndpoint(
    ...
    /**
     * 注释  向RPC注册master终端，涉及到Master的实例化
     * rpcEnv.setupEndpoint 启动Endpoint
