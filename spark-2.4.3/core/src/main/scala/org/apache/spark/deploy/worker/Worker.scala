...
private[deploy] class Worker(
   ...
   * 3、启动work的 WorkerWebUI
   * 4、worker 启动了后要向Master 汇报 registerWithMaster() （***）
   */
      ...
      registerMasterThreadPool.submit(new Runnable {
        override def run(): Unit = {
          try {
             ...
             * 获取master的 引用对象
             *
             * 要给master发送消息，必须先获取到master中的 endpoint 的 ref
             */
  ...
  private def sendRegisterMessageToMaster(masterEndpoint: RpcEndpointRef): Unit = {
    /**
     * 
     * RegisterWorker worker的注册消息
     * 所以 Master 的 receive 方法会接收到 RegisterWorker 注册消息
     * Worker 的receive 方法会接收到 Master 返回的注册消息
     */
  ...
  private def handleRegisterResponse(msg: RegisterWorkerResponse): Unit = synchronized {
    msg match {
      case RegisteredWorker(masterRef, masterWebUiUrl, masterAddress) =>
        ...
        /**
         * 
         * 在 Worker 启动后，就开始向Master 进行注册，注册之后，会收到一个RegisterWorkerResponse 的响应消息
         */
        forwordMessageScheduler.scheduleAtFixedRate(new Runnable {
          override def run(): Unit = Utils.tryLogNonFatalError {
            //  发送心跳 给自己
